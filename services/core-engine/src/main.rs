...
async fn write(
    ...
    let t = Instant::now();
    let db = req.database.unwrap_or_else(|| "default".into());
    let affected = if req.params.is_some() {
        req.params.as_ref().unwrap().len() as u64
    } else {
        1
    };
    {
...
async fn engines() -> Json<Vec<EngineInfo>> {
    Json(vec![
        EngineInfo {
            name: "lsm-tree".into(),
            description: "Model-based LSM-Tree with ALICE-Zip compression".into(),
            compression_ratio: "10-50x".into(),
            best_for: "Write-heavy workloads, time-series, IoT".into(),
        },
        EngineInfo {
            name: "btree".into(),
            description: "B-Tree with page-level compression".into(),
            compression_ratio: "3-5x".into(),
            best_for: "Read-heavy OLTP, point queries".into(),
        },
        EngineInfo {
            name: "columnar".into(),
            description: "Column-oriented with procedural compression".into(),
            compression_ratio: "50-1000x".into(),
            best_for: "Analytics, OLAP, data warehousing".into(),
        },
    ])
}
...
async fn stats(State(s): State<Arc<AppState>>) -> Json<StatsResponse> {
    ...
    Json(StatsResponse {
        ...
        total_writes: st.total_writes,
        total_backups: st.total_backups,
        total_replications: st.total_replications,
